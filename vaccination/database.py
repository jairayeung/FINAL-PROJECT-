...
def signup():
    if request.method == "POST":
        ...
        fd = request.form['first_dose']
        vo = request.form['org']
        brand = request.form['brand']
        return redirect(url_for('Registered', fname = firstname, lname = lastname, name = emailad,passwrd = pw, cpasswrd =cp , first = fd, org = vo, b = brand ))
    else:
        ...
        fd = request.args.get['first_dose']
        vo = request.args.get['org']
        brand = request.args.get['brand']
        return redirect(url_for('Registered', fname = firstname, lname = lastname, name = emailad,passwrd = pw, cpasswrd =cp, first = fd, org = vo, b = brand ))

#once registered(check if password matches confirm password and the data here should be added to the database)
@app.route('/registered/<fname>/<lname>/<name>/<passwrd>/cpasswrd/<first>/<org>/<b>/')
